...
function getPlantById(plant_id) {
    ...
    .select('p.user_id', 'p.nickname', 'p.species', 'p.h2o_frequency', 'p.image')
...
async function addPlant(plant) {
    const [newPlantObject] = await db('plants').insert(plant, ['user_id', 'nickname', 'species', 'h20_frequency', 'image'])
...
async function updatePlant(plant_id, info) {
    const [updatedPlant] = await db('plants')
        ...
        .update(info, [
            'user_id', 'nickname', 'species', 'h2o_frequency', 'image',
        ])
        return updatedPlant
}
